...
class MyDefender(Defender):
    def __init__(self, ids: IDS, graph : Graph, choicePath, successPath, policies, epsilon, seed, state_threshold = 0.0, exploit_threshold = 0.0, ib = 1, testDefense = 0, 
                discount = 0.95, weight = 0.5, actor_network_dict = None, testDefenses = 0, numSamples= 1200, omega = [1], RL_policy = None):
    ...
    def rlPolicy(self, alerts, trueAlerts, belief, actor_network, name):
            ...
            for policy in self.policies:
                ...
                for security_anlayst, o in enumerate(omega):
                    ...
                    if "No Investigation" == policy:
                        ...
                        investigation = self.rlPolicy(y_t, y_t_hat, self.beliefDict[policy][name], self.actor_network_dict['training'], name) 
                    elif "All" == policy:
