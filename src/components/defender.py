...
class MyDefender(Defender):
    def __init__(self, ids: IDS, graph : Graph, choicePath, successPath, policies, epsilon, seed, state_threshold = 0.0, exploit_threshold = 0.0, ib = 1, testDefense = 0, 
                discount = 0.95, weight = 0.5, actor_network_dict = None, testDefenses = 0, numSamples= 1200, omega = [1]):
        ...
        self.ib_each_human['All'][0] = 999999
        self.ib['All'][0] = 999999
        self.policy = policy
        self.investigationList = []
        self.investigationList_Y = []
        self.actor_network_dict = actor_network_dict
        self.exploits_for_i = {}
    ...
    def rlPolicy(self, alerts, trueAlerts, belief, actor_network):
        observation = np.concatenate([belief.reshape(-1), np.sum(2**alerts).reshape(-1)], axis= 0)
        policy = actor_network(torch.unsqueeze(torch.tensor(observation, dtype=torch.float), dim = 0))
        investIdx = np.squeeze(torch.argmax(policy).detach().numpy()) 
            ...
            for policy in self.policies:
                ...
                for security_anlayst, o in enumerate(omega):
                    ...
                    if "No Investigation" == policy:
                        ...
                        if name == "untrained":
                            investigation = self.rlPolicy(y_t, y_t_hat, self.beliefDict[policy][name], self.actor_network_dict[name])
                        else:
                            investigation = self.rlPolicy(y_t, y_t_hat, self.beliefDict[policy][name], self.actor_network_dict['training'])    
                    elif "All" == policy:
                ...
                self.beliefDict[policy][name] = self.cal_belief(self.beliefDict[policy][name], p_iiʼl, r_iiʼnkl, investigation)
